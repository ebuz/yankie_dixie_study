export const experimentalLists = [
    [
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "medal",
                        "carrot",
                        "cupcake",
                        "dog"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__7.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "clouds",
                        "scroll",
                        "blocks",
                        "skull"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_11.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "lightning",
                        "bear",
                        "hot_dog",
                        "cup"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__9.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "apple",
                        "log",
                        "visor",
                        "mushroom"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_22.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "purse",
                        "pen",
                        "table",
                        "turkey"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__14.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "dog",
                        "table",
                        "apple",
                        "medal"
                    ],
                    "speaker_order": [
                        3,
                        0,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "pen",
                        "clouds",
                        "carrot",
                        "hot_dog"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "log",
                        "blocks",
                        "mushroom",
                        "cupcake"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "cup",
                        "bear",
                        "purse",
                        "visor"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "skull",
                        "scroll",
                        "turkey",
                        "lightning"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "briefcase",
                        "jewels",
                        "cat",
                        "spine"
                    ],
                    "speaker_order": [
                        3,
                        1,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_1.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "kangaroo",
                        "tree",
                        "helicopter",
                        "acorns"
                    ],
                    "speaker_order": [
                        3,
                        1,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__21.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "hexagon",
                        "oval",
                        "top",
                        "walrus"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_16.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "cake",
                        "castle",
                        "elephant",
                        "wagon"
                    ],
                    "speaker_order": [
                        3,
                        0,
                        2,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__22.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "safe",
                        "onion",
                        "molecule",
                        "cracker"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        2,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_10.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "cake",
                        "cat",
                        "molecule",
                        "castle"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "kangaroo",
                        "spine",
                        "jewels",
                        "hexagon"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "briefcase",
                        "elephant",
                        "top",
                        "safe"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "oval",
                        "wagon",
                        "tree",
                        "acorns"
                    ],
                    "speaker_order": [
                        3,
                        1,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "onion",
                        "walrus",
                        "helicopter",
                        "cracker"
                    ],
                    "speaker_order": [
                        3,
                        1,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "sneaker",
                        "chick",
                        "donkey",
                        "hand"
                    ],
                    "speaker_order": [
                        3,
                        0,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__19.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "ram",
                        "mousetrap",
                        "freighter",
                        "gecko"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_15.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "pig",
                        "sign",
                        "tie",
                        "clock"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__2.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "blimp",
                        "microscope",
                        "dresser",
                        "mirror"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_3.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "flashlight",
                        "horse",
                        "harpoon",
                        "stairs"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__5.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "mousetrap",
                        "sign",
                        "gecko",
                        "chick"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "horse",
                        "flashlight",
                        "tie",
                        "stairs"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "blimp",
                        "ram",
                        "hand",
                        "mirror"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "pig",
                        "clock",
                        "donkey",
                        "dresser"
                    ],
                    "speaker_order": [
                        3,
                        0,
                        2,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "sneaker",
                        "microscope",
                        "harpoon",
                        "freighter"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "belt",
                        "trident",
                        "squirrel",
                        "seismograph"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_20.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "wreath",
                        "pants",
                        "boy",
                        "trunk"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__11.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "satellite",
                        "loom",
                        "mountains",
                        "elk"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_17.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "locket",
                        "frog",
                        "cop",
                        "tricycle"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__4.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "judge",
                        "ear",
                        "printer",
                        "fan"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_5.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "pants",
                        "tricycle",
                        "trunk",
                        "trident"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "cop",
                        "ear",
                        "printer",
                        "belt"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        2,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "fan",
                        "satellite",
                        "judge",
                        "mountains"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "elk",
                        "locket",
                        "seismograph",
                        "boy"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "squirrel",
                        "frog",
                        "wreath",
                        "loom"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "organ",
                        "baby",
                        "fish",
                        "knife"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__12.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "fern",
                        "wheel",
                        "bucket",
                        "easel"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_14.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "sun",
                        "hurricane",
                        "car",
                        "piano"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__6.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "foot",
                        "daisy",
                        "circle",
                        "rats"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_8.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "rocket",
                        "box",
                        "iron",
                        "nose"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__3.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "circle",
                        "baby",
                        "sun",
                        "rocket"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "bucket",
                        "wheel",
                        "hurricane",
                        "knife"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "car",
                        "daisy",
                        "iron",
                        "foot"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "nose",
                        "easel",
                        "box",
                        "rats"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "fish",
                        "piano",
                        "organ",
                        "fern"
                    ],
                    "speaker_order": [
                        3,
                        0,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "paratrooper",
                        "leaf",
                        "tweezers",
                        "boat"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_18.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "sock",
                        "groceries",
                        "potato",
                        "jacket"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__13.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "television",
                        "backpack",
                        "pelvis",
                        "calipers"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_12.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "giraffe",
                        "rhino",
                        "winter_hat",
                        "train"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__10.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "gloves",
                        "tripod",
                        "swan",
                        "waffle"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_4.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "jacket",
                        "paratrooper",
                        "tripod",
                        "calipers"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "winter_hat",
                        "waffle",
                        "potato",
                        "groceries"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "tweezers",
                        "sock",
                        "train",
                        "television"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "gloves",
                        "rhino",
                        "swan",
                        "boat"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "pelvis",
                        "leaf",
                        "backpack",
                        "giraffe"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "skunk",
                        "bench",
                        "money",
                        "ballerina"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__18.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "unicycle",
                        "balloon",
                        "can",
                        "spigot"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_21.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "stick",
                        "telephone",
                        "lantern",
                        "helmet"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__20.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "cliff",
                        "volcano",
                        "cactus",
                        "turtles"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_9.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "door",
                        "dinner",
                        "soldier",
                        "cookies"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__15.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "volcano",
                        "ballerina",
                        "turtles",
                        "stick"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "cookies",
                        "unicycle",
                        "can",
                        "cliff"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "balloon",
                        "door",
                        "cactus",
                        "spigot"
                    ],
                    "speaker_order": [
                        3,
                        1,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "telephone",
                        "soldier",
                        "bench",
                        "helmet"
                    ],
                    "speaker_order": [
                        3,
                        1,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "money",
                        "lantern",
                        "skunk",
                        "dinner"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "moon",
                        "utensils",
                        "banana",
                        "blowtorch"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_6.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "lock",
                        "cow",
                        "pedal",
                        "tornado"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        2,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__8.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "shirt",
                        "salamander",
                        "rabbit",
                        "bicycle"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_19.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "hurdle",
                        "water_fountain",
                        "prism",
                        "barn"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__17.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "constellation",
                        "snake",
                        "radish",
                        "window"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_13.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "water_fountain",
                        "radish",
                        "hurdle",
                        "salamander"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "cow",
                        "constellation",
                        "utensils",
                        "moon"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "bicycle",
                        "pedal",
                        "snake",
                        "lock"
                    ],
                    "speaker_order": [
                        3,
                        0,
                        2,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "window",
                        "blowtorch",
                        "barn",
                        "rabbit"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "banana",
                        "prism",
                        "tornado",
                        "shirt"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "grapes",
                        "zebra",
                        "airplane",
                        "heel"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__16.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "toys",
                        "stove",
                        "flippers",
                        "bus"
                    ],
                    "speaker_order": [
                        3,
                        1,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_2.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "postman",
                        "parakeet",
                        "spoon",
                        "bed"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__1.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "saturn",
                        "chair",
                        "tiger",
                        "camera"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_7.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "bed",
                        "tiger",
                        "heel",
                        "parakeet"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        2,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "airplane",
                        "postman",
                        "stove",
                        "bus"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "camera",
                        "chair",
                        "flippers",
                        "zebra"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "toys",
                        "spoon",
                        "saturn",
                        "grapes"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        }
    ],
    [
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "carrot",
                        "dog",
                        "medal",
                        "cupcake"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__7.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "clouds",
                        "scroll",
                        "blocks",
                        "skull"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_11.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "lightning",
                        "hot_dog",
                        "bear",
                        "cup"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__9.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "apple",
                        "log",
                        "visor",
                        "mushroom"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_22.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "table",
                        "turkey",
                        "pen",
                        "purse"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__14.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "dog",
                        "carrot",
                        "cupcake",
                        "apple"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "skull",
                        "table",
                        "mushroom",
                        "blocks"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "log",
                        "turkey",
                        "purse",
                        "bear"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "cup",
                        "hot_dog",
                        "scroll",
                        "visor"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "pen",
                        "medal",
                        "clouds",
                        "lightning"
                    ],
                    "speaker_order": [
                        3,
                        0,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "briefcase",
                        "jewels",
                        "cat",
                        "spine"
                    ],
                    "speaker_order": [
                        3,
                        1,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_1.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "kangaroo",
                        "helicopter",
                        "tree",
                        "acorns"
                    ],
                    "speaker_order": [
                        3,
                        0,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__21.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "hexagon",
                        "oval",
                        "top",
                        "walrus"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_16.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "cake",
                        "wagon",
                        "elephant",
                        "castle"
                    ],
                    "speaker_order": [
                        3,
                        1,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__22.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "safe",
                        "onion",
                        "molecule",
                        "cracker"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        2,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_10.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "cake",
                        "tree",
                        "top",
                        "walrus"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "oval",
                        "castle",
                        "acorns",
                        "wagon"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "onion",
                        "spine",
                        "jewels",
                        "cracker"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "kangaroo",
                        "hexagon",
                        "cat",
                        "helicopter"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "briefcase",
                        "elephant",
                        "safe",
                        "molecule"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "hand",
                        "sneaker",
                        "chick",
                        "donkey"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__19.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "ram",
                        "mousetrap",
                        "freighter",
                        "gecko"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_15.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "clock",
                        "tie",
                        "pig",
                        "sign"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__2.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "blimp",
                        "microscope",
                        "dresser",
                        "mirror"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_3.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "flashlight",
                        "stairs",
                        "harpoon",
                        "horse"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__5.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "mousetrap",
                        "donkey",
                        "chick",
                        "hand"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "blimp",
                        "flashlight",
                        "microscope",
                        "ram"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "pig",
                        "harpoon",
                        "sign",
                        "stairs"
                    ],
                    "speaker_order": [
                        3,
                        1,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "sneaker",
                        "freighter",
                        "mirror",
                        "gecko"
                    ],
                    "speaker_order": [
                        3,
                        1,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "horse",
                        "dresser",
                        "tie",
                        "clock"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "belt",
                        "trident",
                        "squirrel",
                        "seismograph"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_20.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "wreath",
                        "trunk",
                        "boy",
                        "pants"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__11.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "satellite",
                        "loom",
                        "mountains",
                        "elk"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_17.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "locket",
                        "tricycle",
                        "cop",
                        "frog"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__4.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "judge",
                        "ear",
                        "printer",
                        "fan"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_5.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "fan",
                        "mountains",
                        "wreath",
                        "printer"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "squirrel",
                        "loom",
                        "ear",
                        "trunk"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "cop",
                        "seismograph",
                        "frog",
                        "satellite"
                    ],
                    "speaker_order": [
                        3,
                        0,
                        2,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "elk",
                        "trident",
                        "judge",
                        "tricycle"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "pants",
                        "boy",
                        "locket",
                        "belt"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "organ",
                        "fish",
                        "baby",
                        "knife"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__12.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "fern",
                        "wheel",
                        "bucket",
                        "easel"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_14.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "sun",
                        "car",
                        "hurricane",
                        "piano"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        2,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__6.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "foot",
                        "daisy",
                        "circle",
                        "rats"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_8.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "rocket",
                        "iron",
                        "box",
                        "nose"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__3.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "nose",
                        "easel",
                        "rats",
                        "daisy"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        2,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "car",
                        "piano",
                        "baby",
                        "wheel"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "bucket",
                        "knife",
                        "box",
                        "sun"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "circle",
                        "organ",
                        "rocket",
                        "hurricane"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "fish",
                        "iron",
                        "foot",
                        "fern"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "paratrooper",
                        "leaf",
                        "tweezers",
                        "boat"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_18.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "potato",
                        "jacket",
                        "groceries",
                        "sock"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__13.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "television",
                        "backpack",
                        "pelvis",
                        "calipers"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_12.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "giraffe",
                        "train",
                        "winter_hat",
                        "rhino"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__10.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "gloves",
                        "tripod",
                        "swan",
                        "waffle"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_4.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "winter_hat",
                        "waffle",
                        "groceries",
                        "television"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "gloves",
                        "rhino",
                        "backpack",
                        "potato"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "pelvis",
                        "calipers",
                        "train",
                        "paratrooper"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "jacket",
                        "swan",
                        "sock",
                        "giraffe"
                    ],
                    "speaker_order": [
                        3,
                        0,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "tweezers",
                        "leaf",
                        "boat",
                        "tripod"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "skunk",
                        "money",
                        "bench",
                        "ballerina"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__18.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "unicycle",
                        "balloon",
                        "can",
                        "spigot"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_21.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "telephone",
                        "helmet",
                        "stick",
                        "lantern"
                    ],
                    "speaker_order": [
                        3,
                        0,
                        2,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__20.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "cliff",
                        "volcano",
                        "cactus",
                        "turtles"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_9.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "door",
                        "soldier",
                        "dinner",
                        "cookies"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__15.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "cookies",
                        "can",
                        "ballerina",
                        "turtles"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "volcano",
                        "cliff",
                        "unicycle",
                        "skunk"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "balloon",
                        "bench",
                        "door",
                        "stick"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "telephone",
                        "cactus",
                        "lantern",
                        "dinner"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "money",
                        "helmet",
                        "soldier",
                        "spigot"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "moon",
                        "utensils",
                        "banana",
                        "blowtorch"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_6.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "tornado",
                        "lock",
                        "cow",
                        "pedal"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__8.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "shirt",
                        "salamander",
                        "rabbit",
                        "bicycle"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_19.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "hurdle",
                        "barn",
                        "prism",
                        "water_fountain"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__17.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "constellation",
                        "snake",
                        "radish",
                        "window"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_13.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "window",
                        "shirt",
                        "radish",
                        "barn"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "water_fountain",
                        "salamander",
                        "rabbit",
                        "prism"
                    ],
                    "speaker_order": [
                        3,
                        1,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "banana",
                        "pedal",
                        "utensils",
                        "constellation"
                    ],
                    "speaker_order": [
                        3,
                        1,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "cow",
                        "snake",
                        "tornado",
                        "lock"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "bicycle",
                        "blowtorch",
                        "moon",
                        "hurdle"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "grapes",
                        "heel",
                        "airplane",
                        "zebra"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__16.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "toys",
                        "stove",
                        "flippers",
                        "bus"
                    ],
                    "speaker_order": [
                        3,
                        1,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_2.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "bed",
                        "spoon",
                        "postman",
                        "parakeet"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__1.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "saturn",
                        "chair",
                        "tiger",
                        "camera"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_7.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "camera",
                        "bus",
                        "stove",
                        "flippers"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "airplane",
                        "heel",
                        "grapes",
                        "saturn"
                    ],
                    "speaker_order": [
                        3,
                        0,
                        2,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "toys",
                        "parakeet",
                        "tiger",
                        "spoon"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "bed",
                        "chair",
                        "zebra",
                        "postman"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        }
    ],
    [
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "saturn",
                        "chair",
                        "tiger",
                        "camera"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_7.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "postman",
                        "parakeet",
                        "spoon",
                        "bed"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__1.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "toys",
                        "stove",
                        "flippers",
                        "bus"
                    ],
                    "speaker_order": [
                        3,
                        1,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_2.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "grapes",
                        "zebra",
                        "airplane",
                        "heel"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__16.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "constellation",
                        "snake",
                        "radish",
                        "window"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_13.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "camera",
                        "flippers",
                        "saturn",
                        "constellation"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        2,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "bed",
                        "radish",
                        "spoon",
                        "bus"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "toys",
                        "tiger",
                        "snake",
                        "grapes"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "airplane",
                        "parakeet",
                        "zebra",
                        "heel"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "window",
                        "stove",
                        "chair",
                        "postman"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "hurdle",
                        "water_fountain",
                        "prism",
                        "barn"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__17.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "shirt",
                        "salamander",
                        "rabbit",
                        "bicycle"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_19.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "lock",
                        "cow",
                        "pedal",
                        "tornado"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        2,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__8.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "moon",
                        "utensils",
                        "banana",
                        "blowtorch"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_6.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "door",
                        "dinner",
                        "soldier",
                        "cookies"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__15.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "banana",
                        "blowtorch",
                        "salamander",
                        "rabbit"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "water_fountain",
                        "door",
                        "hurdle",
                        "dinner"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "cookies",
                        "barn",
                        "shirt",
                        "lock"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "bicycle",
                        "moon",
                        "soldier",
                        "pedal"
                    ],
                    "speaker_order": [
                        3,
                        0,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "cow",
                        "utensils",
                        "tornado",
                        "prism"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "cliff",
                        "volcano",
                        "cactus",
                        "turtles"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_9.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "stick",
                        "telephone",
                        "lantern",
                        "helmet"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__20.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "unicycle",
                        "balloon",
                        "can",
                        "spigot"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_21.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "skunk",
                        "bench",
                        "money",
                        "ballerina"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__18.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "gloves",
                        "tripod",
                        "swan",
                        "waffle"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_4.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "money",
                        "lantern",
                        "stick",
                        "unicycle"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "gloves",
                        "spigot",
                        "skunk",
                        "cactus"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "balloon",
                        "tripod",
                        "waffle",
                        "can"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "volcano",
                        "bench",
                        "helmet",
                        "cliff"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "telephone",
                        "ballerina",
                        "turtles",
                        "swan"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "giraffe",
                        "rhino",
                        "winter_hat",
                        "train"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__10.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "television",
                        "backpack",
                        "pelvis",
                        "calipers"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_12.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "sock",
                        "groceries",
                        "potato",
                        "jacket"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__13.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "paratrooper",
                        "leaf",
                        "tweezers",
                        "boat"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_18.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "rocket",
                        "box",
                        "iron",
                        "nose"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__3.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "nose",
                        "calipers",
                        "backpack",
                        "paratrooper"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "pelvis",
                        "potato",
                        "television",
                        "groceries"
                    ],
                    "speaker_order": [
                        3,
                        1,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "winter_hat",
                        "boat",
                        "sock",
                        "box"
                    ],
                    "speaker_order": [
                        3,
                        1,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "tweezers",
                        "train",
                        "leaf",
                        "rocket"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "jacket",
                        "iron",
                        "rhino",
                        "giraffe"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "foot",
                        "daisy",
                        "circle",
                        "rats"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_8.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "sun",
                        "hurricane",
                        "car",
                        "piano"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__6.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "fern",
                        "wheel",
                        "bucket",
                        "easel"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_14.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "organ",
                        "baby",
                        "fish",
                        "knife"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__12.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "judge",
                        "ear",
                        "printer",
                        "fan"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_5.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "fish",
                        "sun",
                        "baby",
                        "hurricane"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "circle",
                        "judge",
                        "daisy",
                        "knife"
                    ],
                    "speaker_order": [
                        3,
                        0,
                        2,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "fan",
                        "easel",
                        "ear",
                        "organ"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "car",
                        "printer",
                        "fern",
                        "foot"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "bucket",
                        "rats",
                        "piano",
                        "wheel"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        2,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "locket",
                        "frog",
                        "cop",
                        "tricycle"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__4.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "satellite",
                        "loom",
                        "mountains",
                        "elk"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_17.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "wreath",
                        "pants",
                        "boy",
                        "trunk"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__11.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "belt",
                        "trident",
                        "squirrel",
                        "seismograph"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_20.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "flashlight",
                        "horse",
                        "harpoon",
                        "stairs"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__5.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "horse",
                        "satellite",
                        "belt",
                        "wreath"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "elk",
                        "locket",
                        "seismograph",
                        "boy"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "pants",
                        "mountains",
                        "flashlight",
                        "trident"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "cop",
                        "harpoon",
                        "frog",
                        "tricycle"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "squirrel",
                        "loom",
                        "trunk",
                        "stairs"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "blimp",
                        "microscope",
                        "dresser",
                        "mirror"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_3.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "pig",
                        "sign",
                        "tie",
                        "clock"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__2.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "ram",
                        "mousetrap",
                        "freighter",
                        "gecko"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_15.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "sneaker",
                        "chick",
                        "donkey",
                        "hand"
                    ],
                    "speaker_order": [
                        3,
                        0,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__19.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "safe",
                        "onion",
                        "molecule",
                        "cracker"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        2,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_10.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "sneaker",
                        "ram",
                        "tie",
                        "cracker"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "mousetrap",
                        "dresser",
                        "safe",
                        "clock"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "pig",
                        "freighter",
                        "mirror",
                        "hand"
                    ],
                    "speaker_order": [
                        3,
                        0,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "onion",
                        "molecule",
                        "gecko",
                        "sign"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "blimp",
                        "donkey",
                        "microscope",
                        "chick"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "cake",
                        "castle",
                        "elephant",
                        "wagon"
                    ],
                    "speaker_order": [
                        3,
                        0,
                        2,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__22.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "hexagon",
                        "oval",
                        "top",
                        "walrus"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_16.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "kangaroo",
                        "tree",
                        "helicopter",
                        "acorns"
                    ],
                    "speaker_order": [
                        3,
                        1,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__21.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "briefcase",
                        "jewels",
                        "cat",
                        "spine"
                    ],
                    "speaker_order": [
                        3,
                        1,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_1.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "purse",
                        "pen",
                        "table",
                        "turkey"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__14.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "cake",
                        "spine",
                        "top",
                        "hexagon"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "briefcase",
                        "acorns",
                        "elephant",
                        "table"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "kangaroo",
                        "walrus",
                        "helicopter",
                        "purse"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "oval",
                        "jewels",
                        "tree",
                        "castle"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "pen",
                        "wagon",
                        "cat",
                        "turkey"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "apple",
                        "log",
                        "visor",
                        "mushroom"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_22.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "lightning",
                        "bear",
                        "hot_dog",
                        "cup"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__9.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "clouds",
                        "scroll",
                        "blocks",
                        "skull"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_11.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "medal",
                        "carrot",
                        "cupcake",
                        "dog"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__7.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "cup",
                        "mushroom",
                        "visor",
                        "apple"
                    ],
                    "speaker_order": [
                        3,
                        1,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "log",
                        "medal",
                        "clouds",
                        "carrot"
                    ],
                    "speaker_order": [
                        3,
                        1,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "dog",
                        "blocks",
                        "hot_dog",
                        "cupcake"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "skull",
                        "lightning",
                        "bear",
                        "scroll"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        }
    ],
    [
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "saturn",
                        "chair",
                        "tiger",
                        "camera"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_7.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "bed",
                        "spoon",
                        "postman",
                        "parakeet"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__1.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "toys",
                        "stove",
                        "flippers",
                        "bus"
                    ],
                    "speaker_order": [
                        3,
                        1,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_2.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "grapes",
                        "heel",
                        "airplane",
                        "zebra"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__16.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "constellation",
                        "snake",
                        "radish",
                        "window"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_13.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "window",
                        "spoon",
                        "postman",
                        "grapes"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "camera",
                        "tiger",
                        "snake",
                        "flippers"
                    ],
                    "speaker_order": [
                        3,
                        0,
                        2,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "airplane",
                        "zebra",
                        "radish",
                        "chair"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "toys",
                        "stove",
                        "saturn",
                        "constellation"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "bed",
                        "bus",
                        "parakeet",
                        "heel"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        2,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "hurdle",
                        "barn",
                        "prism",
                        "water_fountain"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__17.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "shirt",
                        "salamander",
                        "rabbit",
                        "bicycle"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_19.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "tornado",
                        "lock",
                        "cow",
                        "pedal"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__8.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "moon",
                        "utensils",
                        "banana",
                        "blowtorch"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_6.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "door",
                        "soldier",
                        "dinner",
                        "cookies"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__15.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "banana",
                        "moon",
                        "hurdle",
                        "soldier"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "bicycle",
                        "blowtorch",
                        "barn",
                        "pedal"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "water_fountain",
                        "rabbit",
                        "shirt",
                        "dinner"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "cow",
                        "utensils",
                        "prism",
                        "salamander"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "cookies",
                        "tornado",
                        "lock",
                        "door"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "cliff",
                        "volcano",
                        "cactus",
                        "turtles"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_9.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "telephone",
                        "helmet",
                        "stick",
                        "lantern"
                    ],
                    "speaker_order": [
                        3,
                        0,
                        2,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__20.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "unicycle",
                        "balloon",
                        "can",
                        "spigot"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_21.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "skunk",
                        "money",
                        "bench",
                        "ballerina"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__18.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "gloves",
                        "tripod",
                        "swan",
                        "waffle"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_4.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "balloon",
                        "skunk",
                        "spigot",
                        "stick"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "gloves",
                        "turtles",
                        "tripod",
                        "lantern"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "money",
                        "swan",
                        "ballerina",
                        "cliff"
                    ],
                    "speaker_order": [
                        3,
                        0,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "telephone",
                        "helmet",
                        "waffle",
                        "can"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "volcano",
                        "unicycle",
                        "bench",
                        "cactus"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "giraffe",
                        "train",
                        "winter_hat",
                        "rhino"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__10.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "television",
                        "backpack",
                        "pelvis",
                        "calipers"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_12.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "potato",
                        "jacket",
                        "groceries",
                        "sock"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__13.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "paratrooper",
                        "leaf",
                        "tweezers",
                        "boat"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_18.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "rocket",
                        "iron",
                        "box",
                        "nose"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__3.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "pelvis",
                        "television",
                        "rhino",
                        "train"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "winter_hat",
                        "paratrooper",
                        "rocket",
                        "sock"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "nose",
                        "iron",
                        "backpack",
                        "leaf"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "jacket",
                        "box",
                        "boat",
                        "calipers"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "tweezers",
                        "potato",
                        "groceries",
                        "giraffe"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "foot",
                        "daisy",
                        "circle",
                        "rats"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_8.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "sun",
                        "car",
                        "hurricane",
                        "piano"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        2,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__6.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "fern",
                        "wheel",
                        "bucket",
                        "easel"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_14.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "organ",
                        "fish",
                        "baby",
                        "knife"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__12.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "judge",
                        "ear",
                        "printer",
                        "fan"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_5.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "fish",
                        "ear",
                        "organ",
                        "sun"
                    ],
                    "speaker_order": [
                        3,
                        1,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "circle",
                        "judge",
                        "baby",
                        "piano"
                    ],
                    "speaker_order": [
                        3,
                        1,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "car",
                        "hurricane",
                        "foot",
                        "knife"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "bucket",
                        "easel",
                        "fern",
                        "printer"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "fan",
                        "rats",
                        "wheel",
                        "daisy"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "locket",
                        "tricycle",
                        "cop",
                        "frog"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__4.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "satellite",
                        "loom",
                        "mountains",
                        "elk"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_17.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "wreath",
                        "trunk",
                        "boy",
                        "pants"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__11.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "belt",
                        "trident",
                        "squirrel",
                        "seismograph"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_20.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "flashlight",
                        "stairs",
                        "harpoon",
                        "horse"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__5.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "squirrel",
                        "mountains",
                        "boy",
                        "stairs"
                    ],
                    "speaker_order": [
                        3,
                        0,
                        2,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "horse",
                        "satellite",
                        "tricycle",
                        "loom"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "elk",
                        "frog",
                        "harpoon",
                        "seismograph"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "pants",
                        "belt",
                        "trident",
                        "flashlight"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        2,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "cop",
                        "trunk",
                        "locket",
                        "wreath"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        1,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "blimp",
                        "microscope",
                        "dresser",
                        "mirror"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_3.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "clock",
                        "tie",
                        "pig",
                        "sign"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__2.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "ram",
                        "mousetrap",
                        "freighter",
                        "gecko"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_15.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "hand",
                        "sneaker",
                        "chick",
                        "donkey"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__19.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "safe",
                        "onion",
                        "molecule",
                        "cracker"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        2,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_10.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "onion",
                        "chick",
                        "molecule",
                        "clock"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "blimp",
                        "mirror",
                        "cracker",
                        "ram"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "sneaker",
                        "dresser",
                        "tie",
                        "freighter"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "mousetrap",
                        "sign",
                        "donkey",
                        "gecko"
                    ],
                    "speaker_order": [
                        1,
                        2,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "pig",
                        "microscope",
                        "safe",
                        "hand"
                    ],
                    "speaker_order": [
                        0,
                        2,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "cake",
                        "wagon",
                        "elephant",
                        "castle"
                    ],
                    "speaker_order": [
                        3,
                        1,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__22.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "hexagon",
                        "oval",
                        "top",
                        "walrus"
                    ],
                    "speaker_order": [
                        1,
                        3,
                        2,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_16.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "kangaroo",
                        "helicopter",
                        "tree",
                        "acorns"
                    ],
                    "speaker_order": [
                        3,
                        0,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__21.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "briefcase",
                        "jewels",
                        "cat",
                        "spine"
                    ],
                    "speaker_order": [
                        3,
                        1,
                        0,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_1.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "table",
                        "turkey",
                        "pen",
                        "purse"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/normal__14.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "briefcase",
                        "acorns",
                        "jewels",
                        "spine"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        0,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "kangaroo",
                        "helicopter",
                        "elephant",
                        "turkey"
                    ],
                    "speaker_order": [
                        3,
                        0,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "cake",
                        "cat",
                        "walrus",
                        "tree"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        3,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "oval",
                        "purse",
                        "wagon",
                        "hexagon"
                    ],
                    "speaker_order": [
                        2,
                        0,
                        3,
                        1
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "pen",
                        "castle",
                        "top",
                        "table"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        3,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "partner",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "apple",
                        "log",
                        "visor",
                        "mushroom"
                    ],
                    "speaker_order": [
                        2,
                        3,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_low_frequency_filler_22.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "lightning",
                        "hot_dog",
                        "bear",
                        "cup"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__9.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "clouds",
                        "scroll",
                        "blocks",
                        "skull"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/filler_filler_11.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "carrot",
                        "dog",
                        "medal",
                        "cupcake"
                    ],
                    "speaker_order": [
                        0,
                        3,
                        1,
                        2
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": "recordings/mock_recordings/slang__7.ogg",
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        },
        {
            "role": "speaker",
            "instructions": {
                "finished_instructions": false,
                "forSpeaker": "You will see four pictures. Your partner will see the same set of pictures but in a different order. Start recording yourself and name or describe the picture using a single word, starting with the pictures on the left. When you end your recording, it will be played to your partner. Your partner will try to click on the pictures in the order you say them. Once your partner has clicked on all the pictures the turn will end and another will begin.",
                "forPartner": "You will see four pictures on the upper half of your screen. Your partner sees the same set of pictures but in a different order. Your partner will record themselves naming or describing each picture using a single word in the order that they see them. After they finish, you will hear that recording. Click on the picture that corresponds to each word in the order that you hear them. When you click on a picture it will move to the lower half of your screen. Once you select the last picture the turn will end and another will begin."
            },
            "trials": [
                {
                    "stimuli": [
                        "skull",
                        "lightning",
                        "blocks",
                        "carrot"
                    ],
                    "speaker_order": [
                        3,
                        2,
                        1,
                        0
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "cup",
                        "scroll",
                        "clouds",
                        "bear"
                    ],
                    "speaker_order": [
                        0,
                        1,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "dog",
                        "visor",
                        "medal",
                        "cupcake"
                    ],
                    "speaker_order": [
                        1,
                        0,
                        2,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                },
                {
                    "stimuli": [
                        "log",
                        "mushroom",
                        "apple",
                        "hot_dog"
                    ],
                    "speaker_order": [
                        2,
                        1,
                        0,
                        3
                    ],
                    "response": [],
                    "displayed_pictures": false,
                    "completed": false,
                    "instructions_played": false,
                    "mock_recording": null,
                    "speaker_recording": null,
                    "partner_rt_adjust": null,
                    "data": {}
                }
            ]
        }
    ]
]
